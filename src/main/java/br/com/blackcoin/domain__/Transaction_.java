...
public class Transaction_ {
	...
	Transaction_(String fromKey, String toKey, BigDecimal amount) {
	...
	private void signTransaction(PrivateKey privateKey) throws Exception {
        if (!fromKey.equals(CryptoUtil_.getStringFromKey(privateKey))) {
	...
	public boolean isValid() {
        ...
        try {
            PublicKey publicKey = CryptoUtil_.getKeyFromString(fromKey);
            return CryptoUtil_.verifySignature(publicKey, calculateHash(), Hex.decode(signature));
        } catch (Exception e) {
